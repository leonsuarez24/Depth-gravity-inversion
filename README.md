# Cerro Machín Synthetic Gravity Experiment

This directory contains the synthetic gravity-inversion experiment based on the
Cerro Machín volcanic system (`modelA`). The experiment evaluates whether a
compositional Deep Image Prior (DIP) can reconstruct a three-dimensional signed
density-contrast model from noise-free surface gravity observations.
...
The survey contains 676 gravity receivers. The noise-free synthetic observations
are calculated with
...
The required input files are located in `data_obs_exps/modelA/`:
...
## Compositional Deep Image Prior
...
2. generates the noise-free gravity observations;
3. visualizes the true model and survey response;
4. defines every component of the compositional DIP directly in the notebook;
5. constructs and trains the compositional DIP;
...
considered together. In the full experiment, the compositional DIP is expected
to recover coherent signed bodies with a depth distribution close to the true
Cerro Machín synthetic model. The paper reports 27.15 dB PSNR, 35.12 kg/m³
model RMSE, 0.873 SSIM, and 0.071 mGal receiver RMSE for this experiment.
